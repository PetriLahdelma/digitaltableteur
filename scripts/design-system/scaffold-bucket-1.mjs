...
export const COMPONENTS_DATA = [
  ...
  },
  {
    ...
    element: "span",
    description:
      "Small static or removable label for metadata (categories, statuses, filters). Distinct from `Badge` (which is the design-system semantic chip with stable contracts) — `Tag` is the lightweight catalog primitive used by editorial surfaces.",
    variants: {
      variant: {
        values: [
          "default",
    ...
    keyboard:
      "Static: none. Removable: dismiss control is a real button — Enter/Space activates, Tab moves to it from the surrounding context.",
    pointer:
      "Click on the dismiss control removes the tag; the body of the tag is non-interactive unless wrapped in a link or button by the caller.",
    sr:
      "Static tag announces its text. Removable tag announces 'Remove {label}, button' when the dismiss control is focused.",
    doList: [
      "Use for article categories, filter pills, status chips in lists.",
      "Pair with the matching semantic color when the tag conveys success/warning/error state.",
    ],
    dontList: [
