...
**Catalog completeness** is intentionally below 100%. The ~31% outside the catalog are classified, not forgotten.
...
| **catalog-gap** | **0** (target) | Atoms/molecules that should be cataloged but are not. **File a contract before agent use.** CI should keep this at zero. |
