...
const ContactForm = () => {
  ...
  // In Next.js, process.env is replaced at build time, so we need to access them directly
  // For Next.js, ONLY use process.env.NEXT_PUBLIC_* (build-time replacement)
  const SERVICE_ID =
  ...
  useEffect(() => {
    if (isDev) {
      ...
      console.log("EmailJS Environment Check:", {
        // Raw values (checking availability, not logging actual values)
        "process.env.NEXT_PUBLIC_EMAIL_SERVICE_ID":
          !!process.env.NEXT_PUBLIC_EMAIL_SERVICE_ID,
        "process.env.NEXT_PUBLIC_EMAIL_TEMPLATE_ID":
          !!process.env.NEXT_PUBLIC_EMAIL_TEMPLATE_ID,
        "process.env.NEXT_PUBLIC_EMAIL_PUBLIC_KEY":
          !!process.env.NEXT_PUBLIC_EMAIL_PUBLIC_KEY,
        "import.meta.env available": !!env,
        "env.VITE_EMAIL_SERVICE_ID": !!env?.VITE_EMAIL_SERVICE_ID,
        "env.VITE_EMAIL_TEMPLATE_ID": !!env?.VITE_EMAIL_TEMPLATE_ID,
        "env.VITE_EMAIL_PUBLIC_KEY": !!env?.VITE_EMAIL_PUBLIC_KEY,
        // Resolved values
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const hasAllCredentials = SERVICE_ID && TEMPLATE_ID && PUBLIC_KEY;
    ...
    if (!hasAllCredentials) {
      ...
      // Only log in development or when actually missing (not just falsy)
      if (isDev || (!SERVICE_ID && !TEMPLATE_ID && !PUBLIC_KEY)) {
        // eslint-disable-next-line no-console
        console.error(
      ...
      if (isDev && (!SERVICE_ID || !TEMPLATE_ID || !PUBLIC_KEY)) {
        ...
        console.error(
          "To fix this error:\n" +
            ...
            "   NEXT_PUBLIC_EMAIL_PUBLIC_KEY=***REMOVED*** (for Next.js)\n" +
