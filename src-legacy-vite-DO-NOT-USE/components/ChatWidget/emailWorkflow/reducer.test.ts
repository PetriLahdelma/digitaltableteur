...
describe("emailWorkflowReducer", () => {
  ...
  it("sets fields, advances steps, and edits entries", () => {
    ...
    expect("draft" in withName && withName.draft.fullName).toBe("Jane Doe");
    ...
    expect("draft" in withInterests && withInterests.draft.interest).toEqual([
      "a",
      "b",
      "c",
    ]);
    ...
    const nextWithDraft =
      "draft" in next
        ? next
        : { step: "collectingEmail" as const, draft: createInitialDraft() };
    const review = emailWorkflowReducer(nextWithDraft, { type: "NEXT" });
    const reviewState =
      "draft" in review
        ? { step: "review" as const, draft: review.draft }
        : { step: "review" as const, draft: createInitialDraft() };
    const reviewStep = emailWorkflowReducer(reviewState, { type: "NEXT" });
    // At review, NEXT does not advance past the end of the sequence
    expect(reviewStep.step).toBe("review");

    const edited = emailWorkflowReducer(reviewState, {
      type: "EDIT",
      field: "message",
    });
  ...
  it("handles send request success and errors", () => {
    ...
    expect("errorCode" in error && error.errorCode).toBe("boom");
  });
