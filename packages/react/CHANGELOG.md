...
- `Kbd` gains a `variant` prop (`primary` | `secondary`, type-only export
  ...
  for the previous flatter outlined chip.
- Component stylesheet hygiene: hard-coded phantom custom properties across
  bundled component CSS (Card, Checkbox, CookieConsent, SlideButton and
  others) renamed to their canonical design tokens; rendered values are
  unchanged.
